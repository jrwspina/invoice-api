...
async def get_clients(session: Annotated[AsyncSession, Depends(get_db)]):
    return await db_get_clients(session)


@router.get("/{client_id}/", response_model=ClientRead)
async def get_client(client_id: int, session: Annotated[AsyncSession, Depends(get_db)]):
...
async def create_client(
    client: ClientCreate, session: Annotated[AsyncSession, Depends(get_db)]
):
    return await db_create_client(client, session)
...
async def delete_client(
    client_id: int, session: Annotated[AsyncSession, Depends(get_db)]
):
    client = await db_get_client(client_id, session)
